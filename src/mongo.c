	...
	if (client) {
		if (db_name) {
			...
			if (mongoc_client_command_simple (
				...
				pocket_log_error ("[MONGO] %s", error.message);
			}
		}

		else {
			pocket_log_error ("DB name hasn't been set! Use mongo_set_db_name ()");
		}
	}

	else {
		pocket_log_error ("Not connected to mongo! Call mongo_connect () first");
	}
...
int mongo_connect (void) {
	...
	if (uri_string) {
		...
		if (client) {
			...
			pocket_log_error ("Failed to create a new client instance!\n");
		}
	}

	else {
		pocket_log_error ("Not uri string! Call mongo_set_uri () before attemting a connection");
	}
...
int mongo_collection_drop (mongoc_collection_t *collection) {
	...
	if (mongoc_collection_drop (collection, &error)) {
		...
		pocket_log_error ("Failed to drop collection - %s", error.message);
	}
...
int64_t mongo_count_docs (mongoc_collection_t *collection, bson_t *query) {
	...
	if (collection && query) {
		...
		if (retval < 0) {
			pocket_log_error ("%s", error.message);
...
bool mongo_check (mongoc_collection_t *collection, bson_t *query) {
	...
	if (collection && query) {
		...
		switch (mongoc_collection_count_documents (collection, query, NULL, NULL, NULL, &error)) {
			case -1: pocket_log_error ("%s", error.message); break;
			case 0: break;
...
int mongo_insert_one (mongoc_collection_t *collection, bson_t *doc) {
	...
	if (collection && doc) {
		...
		if (mongoc_collection_insert_one (collection, doc, NULL, NULL, &error)) {
			...
			pocket_log_error ("Insert failed: %s", error.message);
		}
...
int mongo_update_one (mongoc_collection_t *collection, bson_t *query, bson_t *update) {
	...
	if (collection && query && update) {
		...
		if (mongoc_collection_update_one (collection, query, update, NULL, NULL, &error)) {
			...
			pocket_log_error ("Update failed: %s", error.message);
		}
...
int mongo_delete_one (mongoc_collection_t *collection, bson_t *query) {
	...
	if (collection && query) {
		...
		if (mongoc_collection_delete_one (collection, query, NULL, NULL, &error)) {
			...
			pocket_log_error ("Delete failed: %s", error.message);
		}
